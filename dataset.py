...
class DG_Dataset(Dataset):
    def __init__(self, root_dir, domain, split, get_domain_label=False, get_cluster=False, color_jitter=True, min_scale=0.8):
        ...
        self.load_dataset()
        self.intra_index = (torch.randperm(len(self.images))).numpy()
    ...
    def __getitem__(self, index):
        ...
        """
        if self.split == 'train_unlabel':
            image_strong = self.loader(path)
            image_strong = self.strong_transform(image_strong)
            output = [image, image_strong, target]
        else:
            output = [image, target]
        """
    ...
    def set_transform(self, split):
        if split == 'train' or split == 'train_unlabeled':
