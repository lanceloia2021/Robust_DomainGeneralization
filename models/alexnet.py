...
def caffenet(num_classes, num_domains=None, pretrained=False):
    ...
    #
    # if pretrained:
    #     state_dict = torch.load("/home/ruiqi/data/model/alexnet_caffe.pth.tar")
...
class DGcaffenet(nn.Module):
    def __init__(self, num_classes, pretrained=True, grl=True):
...
class DGcaffenet(nn.Module):
    def __init__(self, num_classes, num_domains, pretrained=True, grl=True, domain=True):
        super(DGcaffenet, self).__init__()
        self.domain = domain
    ...
    def forward(self, x):
        ...
        #return features, output_class, output_domain
...
def AlexNet(domain=True):
    return DGcaffenet(7, 2, domain=domain)

def test():
    net = AlexNet()
    y = net(Variable(torch.randn(10, 3, 224, 224)))
